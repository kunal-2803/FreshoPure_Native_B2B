...
import { placeOrder ,clearData} from "../redux/slices/Order/index.js";
...
console.log(isSuccess,"isSuccess Kiv alue");
  useEffect(() => {
    if (isSuccess) {
      navigation.dispatch(
        StackActions.replace('orderConfirm')
      );
      dispatch(clearData())
    }
    else{
      console.log(isSuccess,"isSucessValue")
    }
  },[isSuccess]);
  ...
  return (
    <View className="flex">
      ...
      <View className="flex w-full justify-center items-center">
        <View className="flex" style={{ height: windowHeight }}>
          ...
          <View style={{ width: windowWidth * 0.9 }} className="flex flex-row justify-between">
            ...
            <TouchableOpacity onPress={()=>navigation.navigate('address')}><EvilIcons name="chevron-right" size={34} /></TouchableOpacity>
          </View>


          <View style={{ width: windowWidth * 0.9 }}>
            <View className="bg-white h-14 rounded-lg flex flex-row px-1 justify-between items-center my-1" style={{
